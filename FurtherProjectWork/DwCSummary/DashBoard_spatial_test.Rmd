...
selectedData1 <- reactive({
  ...
  XCP1<-subset(X,X$coordinatePrecision<=num1())
    
    c_1<-ddply(XCP1,~coordinatePrecision,summarise,number_of_distinct_orders=length((coordinatePrecision)))
...
```{r}
  ...
  XCU<-subset(X,X$coordinatePrecision<=num1())
  if(nrow(XCU)>0)
  {
  c<-ggmap(mapgilbert) +
    geom_point(data = XCU, aes(x = decimalLongitude, y = decimalLatitude, alpha = 0.5, size =coordinatePrecision, colour=coordinatePrecision ),shape=20)
  print(c)
    } else{
