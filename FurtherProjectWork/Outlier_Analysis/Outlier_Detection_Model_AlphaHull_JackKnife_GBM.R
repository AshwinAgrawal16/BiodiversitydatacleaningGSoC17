...
Model_Outlier_SVM<-function(X){
  ...
  if(Val2<=(Val1/2)){
    ...
    # Now applying the gbm model
    trainset$V20 <- ifelse(trainset$V20 == 1, "Yes","No")
    gbmFit1 <- train(as.factor(V20) ~ ., data = trainset, method = "gbm", trControl = fitControl,verbose = FALSE)
    
    gbm_ITV2 <- predict(gbmFit1, testset,type= "prob")[,2]
    
    # Checking the results for 95% confidence interval
    fitted.results <- ifelse(gbm_ITV2  > 0.95,1,0)
    fitted.results
